...
static void print_data_handler(unsigned char *handler)
{
    ...
    for (t = 0; t < 4; t++) {
        if ((handler[t] >= 'a' && handler[t] <= 'z') ||
            (handler[t] >= 'A' && handler[t] <= 'Z') || (handler[t] >= '0' && handler[t] <= '9')) {
            printf("%c", handler[t]);
        } else {
...
static int parse_hdrl_list(FILE * in, struct avi_header_t *avi_header,
...
static int parse_hdrl(FILE * in, struct avi_header_t *avi_header,
                      struct stream_header_t *stream_header, struct stream_format_t *stream_format,
                      unsigned int size)
{
    char chunk_id[5];
    int chunk_size;
